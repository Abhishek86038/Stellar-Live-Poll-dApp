...
// CONSTANTS - HARDCODED FOR ABSOLUTE RELIABILITY
...
export const buildTransaction = async (walletAddress, contractAddress, method, args) => {
  try {
    ...
    // EXPLICIT NETWORK PASSPHRASE IN BUILDER
    const tx = new StellarSdk.TransactionBuilder(sourceAccount, {
      fee: "1500", // Slightly higher for reliability
...
export const submitToTestnet = async (signedXdr) => {
  try {
    // Extract XDR string safely
    let xdrStr = typeof signedXdr === 'string' ? signedXdr : (signedXdr.signedTransaction || signedXdr.xdr || "");
    if (!xdrStr) throw new Error("No transaction XDR received");

    // RE-BUILD WITH EXPLICIT NETWORK FOR FINAL SUBMISSION
    const tx = StellarSdk.TransactionBuilder.fromXDR(xdrStr, TESTNET_PASSPHRASE);
    const response = await server.sendTransaction(tx);

    if (response.status === "ERROR") throw new Error("TX Rejected by network");
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (status.status === "FAILED") throw new Error("Contract execution failed on ledger");
...
export const castVote = async (walletAddress, contractAddress, optionIndex) => {
  try {
    ...
    // SIGN with explicit network requirement
